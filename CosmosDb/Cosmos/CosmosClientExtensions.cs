...
namespace CosmosDb
{
    public static class CosmosClientExtionsions
    {
        //TODO: Log queries and performance

        //#region DocumentDB API

        //public static async Task<CosmosResponse> InsertDocument<T>(this CosmosClient c, T document)
        //{
        //    try
        //    {
        ...
        public static async Task<CosmosContainer> GetCosmosContainer(string connectionString, string databaseName, string collectionName, bool forceCreate = true, int initialRU = 400, string partitionKeyPropertyName = "PartitionKey")
        {
            var client = await GetCosmosClient(collectionName);


            return client.GetContainer(databaseName, collectionName);
        }

        public static async Task<CosmosClient> GetCosmosClient(string connectionString)
        {
            var cco = new CosmosClientOptions(connectionString)
            {
                ConnectionMode = ConnectionMode.Direct,
                RequestTimeout = new TimeSpan(1, 0, 0),
                MaxRetryAttemptsOnThrottledRequests = 10,
            };
            var client = new CosmosClient(cco);

            return client;
        }

        public static async Task<CosmosClient> GetCosmosClient(string accountName, string key)
        {
            var accountEndpoint = $"{accountName}";
            var cco = new CosmosClientOptions(accountEndpoint, key)
            {
                ConnectionMode = ConnectionMode.Direct,
                RequestTimeout = new TimeSpan(1, 0, 0),
                MaxRetryAttemptsOnThrottledRequests = 10,
